...
if (isset($_POST['submit'])) {
    ...
    if (!empty($email) and !empty($password) and !empty($confirmPassword)) {
        ...
        $checkEmailExist = $databaseConnexion->prepare('SELECT email from users where email = ?');
        ...
        if ($checkEmailExist->rowCount() >= 1) {
            ...
            if ($password == $confirmPassword) {
                ...
                $saveUser = $databaseConnexion->prepare('INSERT INTO users(email, password) VALUES(?,?)');
